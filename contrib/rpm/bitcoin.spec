...
%if %{_buildqt}
# qt icons
install -D -p share/pixmaps/rarecoin.ico %{buildroot}%{_datadir}/pixmaps/rarecoin.ico
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 7559
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 7559
